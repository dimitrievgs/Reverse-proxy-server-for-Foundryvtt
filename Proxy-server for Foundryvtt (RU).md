...
3) В сравнении с размещением Foundryvtt на личном компьютере и расшариванием доступа к нему через nginx, Вам не нужно проводить заново настройку личного компьютера для пересылки данных по сети в случае переустановки операционной системы, логика общения по сети расположена на стороне vps (не считая ssh и wireguard подключения). Вам достаточно будет установить wireguard, добавить в нём конфигурацию и открыть UDP-порт в файрволе. Дополнительная настройка Foundryvtt не требуется.
5) В сравнении с размещением Foundryvtt на личном компьютере и расшариванием доступа к нему через nginx, здесь на личном компьютере не нужно открывать TCP-порты 80 (http), 443 (https).

...
2) Готовиться к играм Вы в этом случае будете с вашего компьютера, не тратя время на получение и пересылку трафика, как если бы было, если бы сам Foundryvtt был размещён на удалённом арендуемом сервере. 
3) Также, Вам не нужно проводить заново настройку личного компьютера для пересылки данных по сети в случае переустановки операционной системы. Все основные настройки сделаны на стороне vps, и Вам достаточно будет установить wireguard, добавить в нём конфигурацию и открыть UDP-порт в файрволе. Дополнительная нсатройка Foundryvtt не требуется.

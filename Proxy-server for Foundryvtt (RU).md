...
2) В сравнении с размещением Foundryvtt на удалённом арендуемом сервере, готовиться к играм Вы в этом случае будете с вашего компьютера, не тратя время на получение и пересылку трафика (если Вы держите личный сервер дома, то, конечно, задержки минимизированы). 
4) В сравнении с размещением Foundryvtt на личном домашнем сервере, не нужно беспокоиться о пожарной безопасности, что было бы разумным, если бы Вы держали личный сервер дома и оставляли его включённым.
3) В сравнении с размещением Foundryvtt на личном компьютере и расшаривания доступа к нему через nginx, Вам не нужно проводить заново настройку личного компьютера для пересылки данных по сети в случае переустановки операционной системы, логика общения по сети расположена на стороне vps (не считая ssh и wireguard подключения). Вам достаточно будет установить wireguard, добавить в нём конфигурацию и открыть UDP-порт в файрволе. Дополнительная настройка Foundryvtt не требуется.
5) В сравнении с размещением Foundryvtt на личном компьютере и расшаривания доступа к нему через nginx, здесь на личном компьютере не нужно открывать TCP-порты 80 (http), 443 (https).
...
1) Игроки будут иметь доступ к Foundryvtt только тогда, когда у вас включён компьютер, и запущены и подключены wireguard и Foundryvtt.

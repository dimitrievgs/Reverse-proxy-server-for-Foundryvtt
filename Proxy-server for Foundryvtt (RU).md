...
4) В сравнении с размещением Foundryvtt на персональном компьютере и расшариванием доступа к Foundryvtt с помощью nginx, Вам не нужно проводить заново настройку личного компьютера для пересылки данных по сети в случае переустановки операционной системы, логика общения по сети расположена на стороне vps (не считая ssh и wireguard подключения). Вам достаточно будет установить wireguard, добавить в нём конфигурацию и открыть UDP-порт в файрволе. Дополнительная настройка Foundryvtt не требуется. Таким образом, процесс настройки минимальным образом зависит от конкретной операционной системы, которая установлена на персональных компьютерах ГМов, которые используют этот сервер.

...
[причешу потом] Преимуществами данного подхода является то, что:
1) В случае необходимости аренды сервера Вы можете минимизировать затраты, т.к. для проксирования трафика достаточно слабой машины. 
2) Готовиться к играм Вы в этом случае будете с вашего компьютера, не тратя время на получение и пересылку трафика, как если бы было, если бы сам Foundryvtt был размещён на удалённом арендуемом сервере (если Вы держите личный сервер дома, то, конечно, задержки минимизированы). 
3) Также, Вам не нужно проводить заново настройку личного компьютера для пересылки данных по сети в случае переустановки операционной системы, логика общения по сети расположена на стороне vps (не считая ssh и wireguard подключения). Вам достаточно будет установить wireguard, добавить в нём конфигурацию и открыть UDP-порт в файрволе. Дополнительная настройка Foundryvtt не требуется.
...
5) Не нужно открывать TCP-порты 80 (http), 443 (https) на компьютере, на котором непосредственно развёрнут Foundryvtt.
6) Позволяет использовать один сервер vps/vds нескольким ГМам.
